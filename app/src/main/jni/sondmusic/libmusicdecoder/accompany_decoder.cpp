...
#define ALOG_TAG "AccompanyDecoder"
...
int AccompanyDecoder::getMusicMeta(const char* fileString, int * metaData) {
	...
	ALOGI("sampleRate is %d", sampleRate);
...
int AccompanyDecoder::init(const char* audioFile) {
	ALOGI("enter AccompanyDecoder::init");
	audioBuffer = NULL;
	position = -1.0f;
	audioBufferCursor = 0;
	audioBufferSize = 0;
	...
	// 打开输入文件
	ALOGI("open accompany file %s....", audioFile);
	...
	if (result != 0) {
		ALOGI("can't open file %s result is %d", audioFile, result);
		return -1;
	} else {
		ALOGI("open file %s success and result is %d", audioFile, result);
	}
	avFormatContext->max_analyze_duration = 50000;
	//检查在文件中的流的信息
	result = avformat_find_stream_info(avFormatContext, NULL);
	if (result < 0) {
		ALOGI("fail avformat_find_stream_info result is %d", result);
		return -1;
	} else {
		ALOGI("sucess avformat_find_stream_info result is %d", result);
	}
	stream_index = av_find_best_stream(avFormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
	ALOGI("stream_index is %d", stream_index);
	// 没有音频
	if (stream_index == -1) {
		ALOGI("no audio stream");
		return -1;
	}
	//音频流
	...
	// 根据解码器上下文找到解码器
	ALOGI("avCodecContext->codec_id is %d AV_CODEC_ID_AAC is %d", avCodecContext->codec_id, AV_CODEC_ID_AAC);
	AVCodec * avCodec = avcodec_find_decoder(avCodecContext->codec_id);
	if (avCodec == NULL) {
		ALOGI("Unsupported codec ");
		return -1;
	}
	// 打开解码器
	...
	if (!audioCodecIsSupported()) {
		 ...
		 * @param ALOG_offset      ALOGging level offset
		 * @param ALOG_ctx         parent ALOGging context, can be NULL
		 */
		...
		if (!swrContext || swr_init(swrContext)) {
			...
			ALOGI("init resampler failed...");
			return -1;
		}
	}
	ALOGI("channels is %d sampleRate is %d", avCodecContext->channels, avCodecContext->sample_rate);
	pAudioFrame = av_frame_alloc();
//	ALOGI("leave AccompanyDecoder::init");
	return 1;
}
...
//		ALOGI("currentPosition is %.3f", currentPosition);
...
//	ALOGI("enter AccompanyDecoder::readFrame");
	if(seek_req){
		this->seek_frame();
	}
	int ret = 1;
	...
	while (true) {
		readFrameCode = av_read_frame(avFormatContext, &packet);
//		ALOGI(" av_read_frame ...", duration);
		if (readFrameCode >= 0) {
			if (packet.stream_index == stream_index) {
				int len = avcodec_decode_audio4(avCodecContext, pAudioFrame,
						&gotframe, &packet);
//				ALOGI(" avcodec_decode_audio4 ...", duration);
				if (len < 0) {
					ALOGI("decode audio error, skip packet");
				}
				if (gotframe) {
...
//	ALOGI("start destroy!!!");
	...
	if (NULL != avFormatContext) {
		ALOGI("leave LiveReceiver::destory");
